package com.river.module.infra.service.codegen.inner;
...
import com.river.framework.apilog.core.annotation.ApiAccessLog;
...
import com.river.framework.common.util.date.LocalDateTimeUtils;
import com.river.framework.common.util.object.BeanUtils;
import com.river.framework.common.util.object.ObjectUtils;
import com.river.framework.common.util.string.StrUtils;
import com.river.framework.excel.core.annotations.DictFormat;
import com.river.framework.excel.core.convert.DictConvert;
...
import com.river.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 river-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 river-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.river.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/river-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/river-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "river-module-${table.moduleName}/" + // 顶级模块
                "river-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "river-module-${table.moduleName}/" + // 顶级模块
                "river-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "river-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "river-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "river-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "river-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
